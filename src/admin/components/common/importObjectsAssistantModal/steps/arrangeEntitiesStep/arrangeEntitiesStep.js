...
import {
  Form, Header, Button, Modal, Accordion,
...
export const ArrangeEntitiesStep = ({
  definitions, onBack, onNext, header, json, onImport,
}) => {
  ...
  const { source, schema } = useMemo(() => _(definitions)
    ...
    .transform((acc, { definition, entity }) => {
      ...
      const {
        allowReferences, property, matchBy, propertyFilterer = _.stubTrue,
      } = definition;
  ...
  return (
    <>
      ...
      <Modal.Actions className='flex items-center'>
        <span className='mr-auto b'>
          Found
          {utils.joinAndSeparator(_.map(definitions, ({ title, property }) => `${_.size(json[property])} ${_.toLower(title)}`))}
          {' '}
          in the file
        </span>
