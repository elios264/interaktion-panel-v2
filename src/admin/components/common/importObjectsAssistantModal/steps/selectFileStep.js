...
import {
  Segment, Header, Button, Modal, Icon, Dropdown,
...
export const SelectFileStep = ({ definitions, onNext, header }) => {
  ...
  const pickAndValidateFile = (isRestore = false) => dispatch(handleError(async () => {
    ...
    await Promise.all(_.flatMap(requiredDefinitions, ({
      title, property, schema, referenceSchema, allowReferences,
    }) => _.map(json[property], (entity, index) => {
    ...
    _.transform(requiredDefinitions, (theJson, { property, title }) => {

      const collection = theJson[property];
      ...
      theJson[property] = _.keyBy(collection, 'id');
      ...
      <Modal.Content>
        ...
        <Segment placeholder>
          <Header icon>
            ...
            You need to select a JSON file generated by this system that has the same build version:
            {' '}
            {window.__ENVIRONMENT__.BUILD}
          </Header>
          <div className='flex justify-center'>
            <Button.Group primary>
              ...
              <Dropdown
                ...
                floating
                options={[{
                  key: 'restore', active: false, icon: 'undo', text: 'Restore from file', value: 'restore', onClick: () => pickAndValidateFile(true),
                }]}
